...
class StateMunicipality(models.Model):
    ...
    def name_get(self):
        result = super(StateMunicipality, self).name_get()
        result = []
        for record in self:
...
class MunicipalityParish(models.Model):
    ...
    def name_get(self):
        result = super(MunicipalityParish, self).name_get()
        result = []
        for record in self:
            result.append((record.id, "{} ({})".format(record.name, record.municipality_id.name)))
        return result